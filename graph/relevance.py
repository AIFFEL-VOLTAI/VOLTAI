...
class RelevanceRAG:
    def __init__(
        ...
        self.model = ChatOpenAI(model_name=self.model_name, temperature=0.5)
        self.relevance_checker = ChatOpenAI(model=self.model_name, temperature=0.5)
        self.llm_answer_prompt = system_prompt["llm_answer"][f"category{category_number}"]
        self.relevance_check_template = system_prompt["relevance_checker"]

...
class EnsembleRAG:
    def __init__(
        ...
        model_1_name:str="gpt-4o",
        model_2_name:str="gemini-1.5-pro",
        model_3_name:str="gemini-2.0-flash-exp",
        ...
        self.model_3_name = model_3_name

        self.model_name = model_name
        self.model_1 = ChatOpenAI(model_name=self.model_1_name, temperature=0.5)
        self.model_2 = ChatGoogleGenerativeAI(model=self.model_2_name, temperature=0.5)
        self.model_3 = ChatGoogleGenerativeAI(model=self.model_3_name, temperature=0.5)
...
You must generate a final ensemble answer based on the battery research paper PDF, the question, the three LLM answers, and the retrieved document (context). The input consists of:

Retrieved Document (context): the standard answer for battery information.
Answers from three LLM models.
Detailed questions in JSON template format.
Follow these instructions:

For each field, choose the value that best matches the retrieved document.
If there is ambiguity or conflict among the models, decide based on the content of the retrieved document.
The final answer must be a combination of the selected values.
The output must be in JSON format only, with no extra explanations or text.
Please always output the result in JSON array format. Even if the result is a single object, wrap it in an array format []. Please adhere to this format.

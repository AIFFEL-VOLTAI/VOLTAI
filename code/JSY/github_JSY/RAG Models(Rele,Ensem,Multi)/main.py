    ...
    for file_number in file_num_list:
        ...
        for category_number in range(1,5):
            ...
            # ✅ [수정 후 코드]
                ...
                # 2. 메시지의 '내용(content)'만 꺼냅니다. (이게 JSON 문자열입니다)
                json_str = last_msg.content
                
                # 3. 문자열을 파이썬 딕셔너리(또는 리스트)로 변환합니다.
                parsed_data = json.loads(json_str)
                
                # 4. 만약 결과가 리스트([{}])로 묶여있다면 첫 번째 요소를 꺼냅니다.
                if isinstance(parsed_data, list):
                    parsed_data = parsed_data[0]
                
                # 5. 이제 딕셔너리에서 원하는 카테고리 키를 찾습니다.
                temp_answer = parsed_data[category_names[category_number-1]]

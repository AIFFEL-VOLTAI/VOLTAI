...
class RelevanceRAG:
    ...
    def relevance_check(self, state: GraphState) -> GraphState:
        ...
        relevance_chain = prompt | structured_relevance_checker
         ...
         #    llm=self.relevance_checker, target="retrieval-answer").create()
        ...
        # === [수정 시작: 에러 방지 및 디버깅 코드 추가] ===
        # 1. 디버깅을 위해 응답이 실제로 무엇인지 출력해봅니다.
        ...
        if response is not None:
            ...
            # 응답이 None일 경우 (에러 방지용 기본값 설정)
            print("        RELEVANCE CHECK : 응답 생성 실패 (Response is None) -> 'no'로 처리")
            score = "no"  # 흐름이 끊기지 않도록 'no'(관련없음)로 설정
        # === [수정 끝] ===

        # 참고: 여기서의 관련성 평가기는 각자의 Prompt 를 사용하여 수정할 수 있습니다. 여러분들의 Groundedness Check 를 만들어 사용해 보세요!
        return GraphState(relevance=score)

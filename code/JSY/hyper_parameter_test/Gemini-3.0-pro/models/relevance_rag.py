...
class RelevanceRAG:
    def __init__(
        ...
        Return ONLY one token: yes or no. No punctuation, no explanation.
        """
    ...
    def relevance_check(self, state: GraphState) -> GraphState:
        ...
        raw = (prompt | self.relevance_checker).invoke({"context": state["context"], "answer": ans})
        text = getattr(raw, "content", raw)
        text = str(text).strip().lower()

        score = "yes" if text.startswith("yes") else "no"
        print(f"        RELEVANCE CHECK : {score}")
        return {"relevance": score}



    def is_relevant(self, state: GraphState) -> str:
        return state.get("relevance", "no")
